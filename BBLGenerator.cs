...
namespace BibtexSharp
{
    public class BBLGenerator
    {
        ...
        {
            Console.WriteLine("Wczytywanie BIB: {0}", file);

            this.sr = new StringReader(File.ReadAllText(file, Encoding.GetEncoding("Windows-1250")));
            this.row = 1;
            this.col = 1;
            ...
            Console.WriteLine("Liczba pozycji: {0}", this.entries.Count);
        }
        ...
        private Entry ReadBibtexEntry()
        {
            ...
            //  file = {:!literatura\\fuzzy-segm-sciagniete\\Shamir-Human Perception-based Color Segmentation Using Fuzzy Logic .pdf:PDF}
            //}
            ...
            // identyfikator
            string id = this.ReadUntilChar(',').Trim();
            this.ReadChar(); // ,

            Entry ent = new Entry(entry_type, id);
        ...
        {
            Console.WriteLine("Wczytywanie AUX: {0}", file);
            this.sr = new StringReader(File.ReadAllText(file, Encoding.GetEncoding("Windows-1250")));
            this.row = 1;
            this.col = 1;
            ...
            while (true)
            {
                ...
                if (tag == "citation")
                {
                    ...
                    this.citations.Add(key);

                }
            }

            Console.WriteLine("Liczba referencji: {0}", this.citations.Count);
        }
        ...
        internal void Stats()
        {
            ...
            foreach (string key in stat.Keys)
                Console.WriteLine("{0} = {1}", key, stat[key]);
        ...
        {
            Console.WriteLine("Zapis do BBL: {0}", bbl_file_name);
            ...
            using (StreamWriter sw = new StreamWriter(fs, Encoding.GetEncoding("Windows-1250")))
            {
                ...
                foreach (Entry entry in ents)
                {
                    ...
                    switch (entry.Type)
                    {
                        ...
                        default:
                            sw.WriteLine(entry.Key + @"{\bf xxxx}" + entry.Type);
                            break;
                    }
        ...
        private void GenerateMissingCitations(StreamWriter sw, List<Entry> entries, List<string> citations)
        {
            foreach (String key in citations)
            {
                ...
                sw.WriteLine(Boldify("Brak danych dla klucza: " + key));
                sw.WriteLine();
            }
        }
